...
#pragma omp parallel for
			...
			{
				Histogram h_r(truncate, mode);
				Histogram h_g(truncate, mode);
				Histogram h_b(truncate, mode);
				for(int x=0; x<width; x++)
				{
					...
					for(int j=0,idx=0; j<2*r+1; j++)
					{
						uchar* sp = src2.ptr<uchar>(y+j); sp+=3*x;
						...
						for(int i=0; i<2*r+1; i++,idx++)
						{
							...
							addval*= lutc[diff];	
							h_r.add(addval, *(sp+0), metric);
							h_g.add(addval, *(sp+1), metric);
							h_b.add(addval, *(sp+2), metric);
					...
					dst.at<uchar>(y,3*x+0) = h_r.returnVal();
					dst.at<uchar>(y,3*x+1) = h_g.returnVal();
					dst.at<uchar>(y,3*x+2) = h_b.returnVal();
				}
			}
		}
...
{
	weightedHistogramFilter(src,guide,dst,r, sig_bin, sig_s, sig_c, metric, Histogram::BILATERAL);
}

void weightedMedianFilter(Mat& src, Mat& guide, Mat& dst, int r, int truncate, double sig_s, double sig_c, int metric, int sig_bin)
{
	weightedHistogramFilter(src,guide,dst,r, sig_bin, sig_s, sig_c, metric, Histogram::BILATERAL+Histogram::NO_WEIGHT_MEDIAN);
}
